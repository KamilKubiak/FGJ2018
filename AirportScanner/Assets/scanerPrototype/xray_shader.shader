	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i ) : SV_Target
			{
				...
				float2 uv19 = i.ase_texcoord1.xy * float2( 1,1 ) + float2( 0,0 );
				float _speed_Instance = UNITY_ACCESS_INSTANCED_PROP(_speed);
				float mulTime18 = _Time.y * _speed_Instance;
				float2 appendResult17 = (float2(mulTime18 , 0.0));
				#ifdef _ANIMATEDUV_ON
				float2 staticSwitch15 = ( uv19 + appendResult17 );
				#else
				float2 staticSwitch15 = uv19;
				#endif
				...
				float3 tanNormal1 = UnpackNormal( tex2D( _BumpMap, staticSwitch15 ) );
				...
				float2 uv_thicknessMap = i.ase_texcoord1.xy * _thicknessMap_ST.xy + _thicknessMap_ST.zw;
...
/*ASEBEGIN
Version=15800
7;29;1746;1004;1797.802;676.6071;1.424213;True;False
...
ASEEND*/
//CHKSM=0272C321A5D6857BFD3563244D3B11E008C5D69D